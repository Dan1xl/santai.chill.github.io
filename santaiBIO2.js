...
function updateUI(d) {
    ...
    if (activity) {
        console.log('Activity detected:', activity.name, activity.id, activity.assets);
        activityNameEl.innerText = activity.name;
        activityDetailsEl.innerText = activity.details || activity.state || "Играет";
        
        // Обработка изображения с fallback'ами
        if (activity.assets && activity.assets.large_image) {
            const asset = activity.assets.large_image;
            const appId = activity.application_id || activity.applicationId || activity.id;
            const candidates = [];

            if (asset.startsWith('mp:')) {
                candidates.push(`https://media.discordapp.net/${asset.replace('mp:', '')}`);
            } else if (asset.startsWith('external:')) {
                const ext = asset.split('external:')[1];
                // иногда external уже содержит полный URL
                if (ext.startsWith('http')) candidates.push(ext);
                candidates.push(`https://media.discordapp.net/external/${ext}`);
            } else {
                // Популярные варианты для app-assets
                if (asset.startsWith('http')) candidates.push(asset);
                if (appId) {
                    candidates.push(`https://cdn.discordapp.com/app-assets/${appId}/${asset}.png`);
                    candidates.push(`https://cdn.discordapp.com/app-assets/${appId}/${asset}.webp`);
                    candidates.push(`https://cdn.discordapp.com/app-assets/${appId}/${asset}.png?size=1024`);
                }
                // media proxy вариант
                candidates.push(`https://media.discordapp.net/${asset}`);
            }

            let tryIndex = 0;
            let loadTimeout = null;
            const clearHandlers = () => {
                activityImgEl.onerror = null;
                activityImgEl.onload = null;
                if (loadTimeout) { clearTimeout(loadTimeout); loadTimeout = null; }
            };

            const trySetSrc = () => {
                if (tryIndex >= candidates.length) {
                    console.warn('activity image: all candidates failed', { activity, candidates });
                    clearHandlers();
                ...
                console.debug('activity image: trying', url);
                activityImgEl.style.display = 'block';
                activityDefaultIcon.style.display = 'none';
                activityImgEl.src = url;

                // если картинка не загрузилась за 2.5s — пробуем следующий
                loadTimeout = setTimeout(() => {
                    console.warn('activity image: load timeout, trying next', url);
                    trySetSrc();
                }, 2500);
            };

            activityImgEl.onerror = () => {
                console.warn('activity image: onerror, trying next', activityImgEl.src);
                if (loadTimeout) { clearTimeout(loadTimeout); loadTimeout = null; }
                trySetSrc();
            };

            activityImgEl.onload = () => {
                console.debug('activity image: loaded', activityImgEl.src);
                clearHandlers();
                activityImgEl.style.display = 'block';
                activityDefaultIcon.style.display = 'none';
            };
            ...
            // Если нет assets.large_image — показываем подходящую иконку по имени активности
            activityImgEl.style.display = 'none';
            activityDefaultIcon.style.display = 'block';
            activityImgEl.onerror = null;
            ...
            const iconMap = {
                ...
                'Among Us': 'fa-solid fa-ghost',
                'Valorant': 'fa-solid fa-shield-halved',
                'League of Legends': 'fa-solid fa-dragon',
                'Counter-Strike': 'fa-solid fa-crosshairs',
                'Fortnite': 'fa-solid fa-battle-net',
                'Spotify': 'fa-brands fa-spotify'
            };

            const name = activity.name || '';
            let cls = 'fa-solid fa-gamepad';
            ...
            activityDefaultIcon.className = cls + ' activity-placeholder-icon';
        }
        ...
        activityDetailsEl.innerText = "Сплю или отдыхаю...";
        activityImgEl.style.display = 'none';
        activityDefaultIcon.style.display = 'block';
        activityTimeWrapper.style.display = 'none';
    }
